...
class TestMyAgentLangGraph:
    ...
    def test_agent_router_property(self, mock_create_agent, agent):
        """Test that agent_router creates a react agent."""
        mock_llm = Mock()
        with patch.object(MyAgent, "llm", return_value=mock_llm):
            _ = agent.agent_router
    ...
    @patch("agent.myagent.create_agent")
    def test_agent_router_includes_workflow_tools(self, mock_create_agent):
        """Test that agent_router includes workflow_tools alongside mcp_tools."""
        extra_tool = Mock()
        agent = MyAgent(
            api_key="test_key",
            api_base="test_base",
    ...
    @patch("agent.myagent.create_agent")
    def test_agent_analyst_property(self, mock_create_agent, agent):
        """Test that agent_analyst creates a react agent."""
        mock_llm = Mock()
        with patch.object(MyAgent, "llm", return_value=mock_llm):
            _ = agent.agent_analyst
            mock_create_agent.assert_called_once_with(
                mock_llm,
                tools=ANY,
                system_prompt=ANY,
    ...
    @patch("agent.myagent.create_agent")
    def test_agent_reporter_property(self, mock_create_agent, agent):
        """Test that agent_reporter creates a react agent."""
        mock_llm = Mock()
        with patch.object(MyAgent, "llm", return_value=mock_llm):
            _ = agent.agent_reporter
            mock_create_agent.assert_called_once_with(
                mock_llm,
                tools=ANY,
                system_prompt=ANY,
    ...
    def test_workflow_property(self, agent):
        ...
        with patch.object(MyAgent, "llm", return_value=mock_llm):
            with patch("agent.myagent.create_agent"):
                ...
                assert "analyst_node" in workflow.nodes
                assert "reporter_node" in workflow.nodes
